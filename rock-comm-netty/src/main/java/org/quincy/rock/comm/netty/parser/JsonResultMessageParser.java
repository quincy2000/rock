...
public abstract class JsonResultMessageParser<V extends Message> extends JsonMessageParser<CasingResultMessage<V>> {
	 ...
	 */
	public JsonResultMessageParser(Collection<String> contentType) {
		super(contentType);
	}
	 ...
	 * @param contentType 内容类型
	 */
	public JsonResultMessageParser(Collection<Integer> functionCode, Collection<String> contentType) {
	 ...
	 */
	@Override
	public CasingResultMessage<V> fromJson(String message, Map<String, Object> ctx) {
		...
		Message vo = clazz == null ? null : jsonConverter.fromJson(json, clazz);
		//
		json = message.substring(0, leftPos) + "null" + message.substring(rigthPos + 1, message.length());
		CasingResultMessage resultMessage = jsonConverter.fromJson(json, CasingResultMessage.class);
